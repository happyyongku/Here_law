...
class LawUpdateDaemon:
    ...
    def fetch_new_laws(self, target_date: str) -> List[Dict]:
        ...
        while True:
            ...
            if "LawSearch" in response and "law" in response["LawSearch"]:
                result.extend(response["LawSearch"]["law"])
    ...
    def insert_law(self, row, previous_law_id=None):
        ...
        SET previous_law_id = EXCLUDED.previous_law_id;
        """
        ...
        with self.pool.get_connection() as conn:
            with conn.cursor() as cursor:
                ...
                logging.debug(f"insert_law: law_info 삽입/업데이트 성공: {data['law_id']}")
                ...
                logging.debug(f"insert_law: {len(clause_data_list)}개의 law_clause 삽입 성공")

        return law_info['법령ID']
